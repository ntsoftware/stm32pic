...
static bool parse_end_of_file(void)
{
    ctx.eof = true;
    return true;
}

static bool parse_extended_linear_address(void)
{
    if (get_byte_count() != 2)
        return false;
...
static bool validate_line(void)
{
    if (get_start_code() != ':')
        return false;
...
bool parse_hex(const char* hex, size_t n, void* mem, size_t mem_size)
{
